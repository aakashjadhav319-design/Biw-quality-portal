...
if page == "📊 CMM Analytics & Diagnostic Engine":
  ...
  st.caption(
      "Correlates out-of-spec PMP/FD points with tooling locators and incoming"
      " part non-conformances."
  )
  ...
  active_deviations = pd.DataFrame([{
      ...
      "Recommended_Action": (
          "Do NOT shim in-house jig. Quarantine lot 'LOT-202609-A9' and issue"
          " SCAR to SUP-IND-88."
      ),
  }, {
      "CMM_Point": "PMP_UB_02",
      ...
      "Diagnostic_Root_Cause": (
          "🟡 IN-PROCESS JIG DRIFT: Incoming part is on-spec. Wear or loose"
          " bolts detected on MCS-102 locator surface."
      ),
      "Recommended_Action": (
          "Add 0.65mm shim at MCS-102 locator on JIG-UB-01 (Station ST-010) in"
          " Z-axis."
      ),
  }])
  ...
  for idx, row in active_deviations.iterrows():
    with st.expander(
      ...
      with c1:
        ...
        st.write(
            f"**Source Type:** {row['Source_Type']} ({row['Supplier_Code']})"
        )
        st.write(f"**Target Locator:** {row['Locator']}")
      with c2:
        st.write(f"**Root-Cause Diagnosis:** {row['Diagnostic_Root_Cause']}")
...
# ==============================================================================
  ...
  with st.form("jig_correction_form", clear_on_submit=True):
    ...
    remarks = st.text_area(
        ...
        placeholder=(
            "e.g., Added shim to compensate for Z-axis drop following CMM PMP"
            " alert."
        ),
    )
    ...
    if submitted:
      ...
      st.success(
          f"✅ Saved to SQLite (`biw_quality.db`)! Assigned Log ID: {new_log_id}"
      )
...
# ==============================================================================
  ...
  with st.form("supplier_defect_form", clear_on_submit=True):
    ...
    if submitted_sup:
      ...
      st.warning(
          f"🚨 Saved to SQLite (`biw_quality.db`)! Generated Defect ID:"
          f" {new_defect_id}"
      )
